...
if ($content == '') {
	echo '<script>location.href="http://127.0.0.3/add_team_info.php?player_id='.++$player_id.'"</script>';
}
$mysqlData = array(
...
if (!$res) {
    echo '<script>location.href="http://127.0.0.3/add_team_info.php?player_id='.++$player_id.'"</script>';exit;
}
...
foreach ($player[$player_id]['last_ten_info'] as $key => $value) {
    ...
    $sqls = "insert into fa_player_match_data_lol (player_id,date,opp,score,`tower`,dragons,barons,first_blood,is_win,is_fast,scores,remain,season,addtime) values ($player_id,'$value[0]','$opp','$score',$value[7],$value[8],$value[9],$value[6],$value[4],$value[5],$scores,$remain,'2017',$addtime)";
