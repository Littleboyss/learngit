...
class BetAction extends LoginAction{
	...
	*/
    ...
    public function guess(){
        ...
        if($data['is_special'] == 1 && $this->_user['id'] == $data['special_uid'] && $this->_user['is_special'] == 1){
            ...
            //添加我今天投注的次数
            $UserUser->where(array('id' => $this->_user['id']))->setInc('today_guess_num',1);
        }
        $guess_check = true;
        if($data['price'] != 0){
            //减少用户的门票数目
            if($data['is_special'] == 1 && $this->_user['id'] == $data['special_uid'] && $this->_user['is_special'] == 1){
                ...
                // 入帐变表
                $this->insert_account(3,1,$this->_user['id'],$data['price'] * $guess_num,false,$id);
            }
        }
        if($guess_check){
            $match_list = M('MatchRoomInfo')->where(array('room_id' => $id))->find();
            ...
            $all_players = $this->project_players($data['project_id']);
            foreach ($team_info as $key => $value) {
                $user_lineup_info[$key] = $all_players[$value];
            }

            $data['lineup_player_info'] = $user_lineup_info; //返回房间的信息和阵容选手信息
            if($data['tag_img'] == 0){
            ...
            $this->returnMsg(0,'guess',$data,$guess_create_id);
        }else{
    ...
    */
        ...
        $type = $this->_data['type'];//阵容类型 1(推荐) 2(已存)
        if(is_numeric($type)){
            $Map['type'] = $type;
        }
        ...
        $room_info = M('MatchRoom')->where(array('id' => $id))->find();
        if(!$result){
            $this->returnMsg(2,'room');
        }
        ...
        $player_map = $MatchPlayer->getField('id,average,salary');
        ...
        $all_players = $this->project_players($room_info['project_id']); //获取该项目所有的球员或选手信息

        // print_r($all_players);die;
        foreach ($data as $key => $value) {
            $data[$key]['lineup'] = unserialize($value['lineup']);
            $sum = 0;
            $salary_sum = 0;
            foreach ($data[$key]['lineup'] as $k => $v) {
                $sum += $player_map[$v]['average'];
                $salary_sum += $player_map[$v]['salary'];
                $data[$key]['lineup_info'][$k] = $all_players[$v];
            }
            $data[$key]['salary_sum'] = C('MATCH_ROOM_LINEUP')[$value['lineup_id']]['pay'] - $salary_sum;
            $data[$key]['lineup_score'] = $sum/10;
            $data[$key]['project_id'] = $room_info['project_id'];
        }
    ...
    */
    ...
    public function revokelineup(){
        ...
        M('UserUser')->where(array('id' => $this->_user['id']))->setDec('today_guess_num',1);
        if($f1 && $f2){ //返回用户的木头数量
            $check = true;
            if($roomdetail['price'] > 0){
                ...
                $this->insert_account(14,1,$this->_user['id'],$roomdetail['price']*$result['guess_num'],true,$id);
            }
            if($check){
    ...
    public function editlineup(){
        ...
        $uid = $this->_user['id'];
        $UserGuessRecord = M('UserGuessRecord');
        $room_data = $UserGuessRecord->where(array('lineup_id' => $lineup_id,'uid' => $uid))->find();
        // echo $UserGuessRecord->getLastSql();die;
        $id = $room_data['room_id']; //房间的id
        // echo $id;die;
        if(!is_numeric($id)){
            $this->returnMsg(3,'editlineup'); //无法获取已选阵容的房间信息
        }

        if($room_data['status'] == 3){
            $this->returnMsg(4,'editlineup'); //比赛已经结束,无法修改
        }

        //获取原阵容的信息
        $l_lineup_data = M('Lineup')->where(array('id' => $lineup_id))->find();
        if($l_lineup_data['lineup_token'] == md5(unserialize($team_info))){
            $this->returnMsg(7,'editlineup'); //阵容没做任何修改,无法提交
        }
        ...
        foreach ($team_info as $key => $value) { //检测所选阵容是否在房间的选手列表中
            if(!in_array($value, $players_id)){
                $this->returnMsg(5,'editlineup'); //检测所选球员是否在房间球员中
            }

            if($players[$value]['state'] == 1){
                $this->returnMsg(6,'editlineup'); //检测所选球员是否在房间球员中
            }
        ...
        if($result){
            ...
            $this->returnMsg(0,'editlineup',$data,$res);
        }else{
    ...
    public function addguess(){
        ...
        if($data['price'] != 0){
            ...
            if($data['is_special'] == 1 && $this->_user['id'] == $data['special_uid'] && $this->_user['is_special'] == 1){
                ...
                $guess_check = $UserUser->where(array('id' => $this->_user['id']))->setDec('entrance_ticket',$data['price'] * $guess_num);
                // 入帐变表
                $this->insert_account(3,1,$this->_user['id'],$data['price'] * $guess_num,false,$room_id);
            }
        }
        if($guess_check){
            $match_list = M('MatchRoomInfo')->where(array('room_id' => $id))->find();
            ...
            $this->returnMsg(0,'guess',$data, intval($lineup_id));
        }else{
            $this->returnMsg(1,'guess');
        }
    }
