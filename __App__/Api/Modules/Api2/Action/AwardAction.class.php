...
class AwardAction extends LoginAction{
    ...
    public function turnplate_list(){
        ...
        if (!$list) {
            $data =  M('AwardTurnplateList')->field('id,user_name,prize_name,rank,prize_num')->where($where)->order('id desc')->limit(20)->select();
            foreach ($data as $key => $value) {
        ...
        if ($money-$list['cost'] < 0) {
            ...
            if ($res) {
                ...
                if ($result) {
                    ...
                    $array_rank = array(1 =>'一等奖',2 =>'二等奖',3 =>'三等奖',4 =>'四等奖',5 =>'五等奖',6 =>'六等奖');
                    $data['rank'] = $array_rank[$prize['level']];// 奖级
                    $data['prize_num'] = $prize['nums'];// 奖品数量
                    $data['class_id'] = $class_id;// 转盘类型
