...
class RoomAction extends LoginAction{
	...
	*/
	public function roomlist(){
		...
		if(strlen($project_id) > 1){
			$project_ids = explode(',', $project_id);
			$Map['project_id'] = array(array('eq',$project_ids[0]),array('eq',$project_ids[1]),'or');
		}else{
		...
		//是否是推荐的房间
		$is_hot = $this->_data['is_hot'];
		if($is_hot == 1){
			$Map['is_hot'] = 1;
		}
		//是否可以投多注
		$is_more_guess = $this->_data['is_more_guess'];
		if($is_more_guess == 1){
			$Map['allow_uguess_num'] = array('gt',1);
		}

		//是否是必开房间
		$is_open = $this->_data['is_open'];
		if($is_open == 1){
			$Map['open_id'] = 1;
		}
		//排序方式,默认比赛时间正序
		$order = $this->_data['order']; //排序的类型 1比赛时间 2门票 3奖励
		$order_type = $this->_data['order_type']; //排序方式 1 正序 2倒序

		if($order == 1){
			$order_name = 'match_start_time';
		}elseif($order == 2){
			$order_name = 'price';
		}elseif ($order == 3) {
			$order_name = 'reward_num';
		}else{
			$order_name = 'match_start_time';
		}
		...
		//var_dump($MatchRoom->getlastsql());exit;
		...
		foreach ($data as $key => $value) {
			...
			if($value['allow_uguess_num'] > 1){
				$data[$key]['more_guess'] = 1;
			}else{
				$data[$key]['more_guess'] = 2;
			}
		}
		$this->returnMsg(0,'room',$data);
	}
	...
	*/
	...
	public function roomplayer(){
        ...
        // 获取房间所有球员数据
        $data = $this->getroomplayer($id,true);
        // print_r($data);die;
        // 根据房间id判断所属项目
