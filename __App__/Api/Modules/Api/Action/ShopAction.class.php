...
class ShopAction extends LoginAction{
    ...
    public function goods_list(){
        $shop_sub_id = $this->_data['sub_id']; // 分类ID
        $order_price = $this->_data['order'];  // 价格排序
        $is_virtual  = $this->_data['is_virtual'];  // 是否为虚拟商品
        $ShopGoods = M('ShopGoods'); 
        $ShopSub = M('ShopSub'); 
        $map['state']=1;
        $order['id'] = 'asc'; // 默认按照ID从小到大排序
        $data = $this->cache('get','goods_list');
        if (!$data) {
           $data = $ShopGoods->field('id,name,price,type,avatar_img,shop_sub_id,is_virtual')->where($map)->order($order)->select();
           $this->cache('set','goods_list',$data);
        }
        foreach ($data as $key => $value) {
            if ($value['is_virtual'] == $is_virtual) {
                if (!empty($shop_sub_id)) {
                   // 选择分类
                   if ($value['shop_sub_id'] == $shop_sub_id) {
                        $list[] = $value; 
                    }
                }else{
                    $list[] = $value;
        ...
        $extra_data = $ShopSub->field('id,name')->where(array('pid'=>$pid))->select();
        if ($list && $extra_data) {
            $this->returnMsg(0,'turnplate',$list,$extra_data);
        }else{
    ...
    public function order_list(){
        ...
        $res = M('ShopGoodsOrder')->field('t1.id,t1.address_id,t1.numbers,t1.goods_nums,t1.status,t1.price,t2.name,t2.avatar_img,t2.type,t2.shop_sub_id,t1.addtime')->join('as t1 LEFT JOIN '.c('DB_PREFIX').'shop_goods as t2 on t1.goods_id = t2.id')->where(array('t1.user_id'=>$this->_user['id']))->limit($start,10)->order('id desc')->select();
        if ($res) {
    ...
    public function customer_care(){
        $data['user_id']  = $this->_user['id'];      // 用户ID
        $data['question'] = $this->remove_xss($this->_data['question']);// 反馈的问题
        $data['content']  = $this->remove_xss($this->_data['content']); // 问题描述
        $data['addtime']  = time();                  // 添加时间
        if (empty($data['question']) || empty($data['content'])) {
...
LEFT JOIN '.c('DB_PREFIX').'shop_goods AS t2 ON t1.goods_id = t2.id')->where('t2.state = 1 and t1.addtime > '.$one_week)->group(' t2.id')->order(' nums desc')->limit(10)->select();
        if ($res) {
