...
class UserAction extends LoginAction{
	...
	public function userbetmatch(){
		...
		$type_data = array(); //存储房间信息
		$types = array(); //存储一个阵容的所有房间类型
		$_data_ = array(); 
		foreach ($_data as $key => $value) {
			//先把一个阵容下所有的同一个类型的房间放进一个数组
			foreach ($value as $k => $v) {
				$type_data[$v['type_id']][] = $v;
				$types[] = $v['type_id'];
			}

			$hh = array_values(array_unique($types));
			// print_r($hh);
			foreach ($hh as $ko => $vo) {
				$_data_[$key][$ko]['type_id'] = $vo;
				$_data_[$key][$ko]['type_name'] = $type_data[$vo][0]['type_name'];
				$_data_[$key][$ko]['room_list'] = $type_data[$vo];
			}
			unset($type_data);
			unset($types);
			unset($hh);
		...
		foreach ($lineups as $key => $value) {
			...
			$s[$key]['room_info'] = $_data_[$value];
			$s[$key]['project_id'] = $_data_[$value][0]['room_list'][0]['project_id'];
		}
		// print_r($s);die;
	...
	public function userbetmatching(){
		...
		foreach ($lineups as $key => $value) {
			...
			$s[$key]['project_id'] = $_data_[$value][0]['room_list'][0]['project_id'];// 项目id
			...
			$s[$key]['room_info'] = $_data_[$value];// 房间信息
			// 判断是否全部上场
	...
	public function userroomdetail(){
		...
		foreach ($lineup_data as $key => $value) {
			...
			$lineup_player_info[$ii]['postion'] = $key; //球员的位置信息
			$ii++;
		}
		unset($ii);
		...
		if($data['match_status'] != 1){
			...
			// //查询登录用户的20条附近数据
			// $GUMap['lineup_score'] = array('egt',$user_rank['lineup_score']);
			// $all_user_rank = $UserGuessRecord->field('id,uid,is_reward,ranking,lineup_score,lineup_id')->where($GUMap)->order('lineup_score desc')->limit(10)->select(); //查询10条默认数据
			// foreach ($all_user_rank as $key => $value) { //没有做10条数据限制
			// 	if($value['guess_num'] > 1){
			// 		for($i = 1; $i <= $value['guess_num'];$i++){
			// 			$top[] = $value;
			// 		}
			...
			// }
			//查询小于积分的
			// $LUMap['lineup_score'] = array('elt',$user_rank['lineup_score']);
			// $all_user_rank = M('UserGuessRecord')->field('id,uid,is_reward,ranking,lineup_score,lineup_id')->where($GUMap)->order('lineup_score desc')->limit(10)->select(); //查询10条默认数据
			// foreach ($all_user_rank as $key => $value) { //没有做10条数据限制
			// 	if($value['guess_num'] > 1){
			// 		for($i = 1; $i <= $value['guess_num'];$i++){
			// 			$top[] = $value;
			...
			if($room_data['is_special'] == 1 && $room_data['special_uid'] != 0){//主播房,返回主播投注的信息
				...
				// $special_lineup = unserialize($special_data['lineup']); //主播所选的阵容
				// $special_lineup_info = array(); //存储主播所选阵容的信息
				// foreach ($room_data['match_list'] as $k => $v) { //该房间所有比赛信息
				// 	foreach ($special_lineup as $key => $value) {
				// 		if($players[$value]['team_id'] == $v['team_a'] || $players[$value]['team_id'] == $v['team_b']){
				// 			$special_lineup_info[$key]['match_id'] = $v['id']; //塞选该选手属于哪个比赛
				// 		}
				// 		$special_lineup_info[$key]['player_id'] = $value;
				// 	}
				// }

				// foreach ($special_lineup_info as $key => $value) {
				// 	$special_lineup_info['team_info'][$value['player_id']] = $PlayerMatchData->where(array('player_id' => $value['player_id'],'match_id' => $value['match_id']))->find();
	...
	public function get_user_rank(){
		$id = $this->_data['id']; // 称号id
		$rank = M('UserRank');
		$rank = $rank->field('id,name,class_id,avatar_img,depict')->where(array('id'=>$id))->find();
		if ($rank) {
	...
	public function show_all_rank(){
		...
		$data = M('UserRank')->field('t1.id,t1.name,t1.avatar_img,t1.class_id,t2.name as class_name')->join('as t1 left join '.c('DB_PREFIX').'user_rank_class as t2 on t1.class_id = t2.id')->select();
		if ($data) {
			foreach($data as $key => $value){
			...
			$this->returnMsg(0,'room',$last,$datas);// 获取成功
		}else{
			$this->returnMsg(1,'room');// 获取失败
		}
	}
	...
	public function check_bind(){
		...
		if ($open) {
			if ($open['qq_openid'] == '') {
				$open['qq_openid'] = false;
			}
			if ($open['wx_openid'] == '') {
				$open['wx_openid'] = false;
			}
			if ($open['wb_openid'] == '') {
				$open['wb_openid'] = false;
			}
