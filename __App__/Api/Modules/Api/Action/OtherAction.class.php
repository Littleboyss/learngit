...
class OtherAction extends CommonAction{
	// 统计一血获得最多的战队
	public function get_max_first_blood(){
		$wcg = M('MatchPlayerWcg');
		$team = $wcg->where('project_id = 6')->select();
		$match_list = M('PlayerMatchDataDota2');
		foreach ($team as $key => $value) {
			$sum = 0;
			$match_data = $match_list->field('jungle')->where('match_id > 11928 and player_id = '.$value['id'])->select();
			...
			$this->returnMsg(0,'room',$f);
	}
	// 统计总击杀 获得最多的战队
	public function get_max_scores(){
		$wcg = M('MatchPlayerWcg');
		$team = $wcg->where('position < 6 and project_id = 6')->select();
		$match_list = M('PlayerMatchDataDota2');
		foreach ($team as $key => $value) {
			$sum = 0;
			$match_data = $match_list->field('jungle')->where('match_id > 11928 and player_id = 100259')->select();
			foreach ($match_data as $keys => $values) {
				$sum += $values['jungle'];
			}
			if ($sum != 0) {
