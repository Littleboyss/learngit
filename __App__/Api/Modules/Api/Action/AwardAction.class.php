...
class AwardAction extends LoginAction{
        ...
        if (!empty($list['is_get'])) {
            ...
            if(in_array($id, $day)){
                ...
                if ($result) {
                    ...
                    $this->returnMsg(2,'attendance');// 签到奖励领取成功
                }else{
    ...
    public function get_reward(){
        ...
        if(!empty($id)){
            ...
            foreach ($id as $key => $v) {
                if (!in_array($v, $ids)) {
                    $this->returnMsg(2,'reward');// 参数异常
                }else{
                    $value = $UserBetGess->where('id = '.$v)->find();
                    // 判断奖品类型
                    if( $value['type'] == 1){
                        $redcode = 1;
                        $types ='entrance_ticket';
                    ...
                    if ($result) {
                        ...
                        // 入账变表
                        $this->insert_account(4,$redcode,$uid,$value['nums'],true);
                        $award[$key]['nums'] = $value['nums']; 
                        $award[$key]['type'] = $value['type']; 
                    }
                }
            }
            // 统计值
            ...
            $awards[1]['name'] = 'ticket';
            $awards[1]['en_nmus'] = $en_nmus;
            $awards[2]['name'] = 'farm';
            $awards[2]['mu_nmus'] = $mu_nmus;
